...
in
{
  ...
  rec-derived.test-update-replaces-head = {
    expr = R.select (R.update stacked "x" 99) "x";
    expected = 99;
  };

  rec-derived.test-update-preserves-stack-depth = {
    expr = R.depth (R.update stacked "x" 99) "x";
    expected = 2;
  };

  rec-derived.test-update-throws-on-absent = {
  ...
  rec-derived.test-upsert-inserts-when-absent = {
    expr = R.select (R.upsert R.empty "x" 1) "x";
    expected = 1;
  };

  rec-derived.test-upsert-replaces-when-present = {
  ...
  rec-derived.test-show = {
    expr = R.show (R.extend (R.extend R.empty "x" 1) "x" 2);
    expected = "{ x = [2, 1] }";
  };

  rec-derived.test-showCompact = {
