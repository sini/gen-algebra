...
The `search` namespace (Palmer §3's Search monad and its `converge` runner) is **retired**: gen
has one evaluator, gen-scope, and `converge` was a second one, interpreting a caller-authored
continuation program on its own carrier and its own round schedule. Its encoding also departed
