...
| Matching predicates against graph positions — gen-select carries its own conservative-equality dispatch inline instead of importing one (`gen-select/lib/constructors.nix`, bindings `identityOf`/`conservativeEq`; `gen-select/flake.nix` records "no gen-algebra"). It is no longer the one trivial line the earlier note priced | `gen-select` — "gen-select: selector algebra for attributed graph positions" |
...
| `intensionalEq` is unguarded — a bare lambda throws rather than returning `false` | `lib/intensional.nix`, binding `intensionalEq`; `intensionalEq (x: x) (y: y)` ⇒ `error: expected a set but found a function` *(stderr)*. gen-select guards its own relation with an `isIntensional` check first (`gen-select/lib/constructors.nix`, binding `selectorEq`, cross-repo) |
| `converge` dedups intensional continuations by **identity regime**, not by name. Same-key continuations sharing one program point but **behaving differently now BOTH FIRE** — the key is a bucket and membership is decided by Nix `==` on the reified value minus `__id` | `lib/search.nix`, bindings `identityOf` / `classify` / `comparisonSubject` / `dedupContinuations`; two continuations named `"g"` on key `"k"` with different bodies ⇒ **both fire**. RED control, the retired presence loop on the same pair ⇒ **one fires** — the later registration was dropped body and all, silently. Test: `test-intensional-dedup` (`ci/tests/search-converge.nix`) covers the merging arm (one value bound twice ⇒ one survivor) |
...
| The dedup key carries a REGIME TAG between the index key and the payload — `"${key}:m\|s\|u:${payload}"` — so the three arms occupy disjoint spaces | `lib/search.nix`, bindings `regimeTagOf` / `classify`; an unmigrated continuation NAMED string-equal to another's minted digest fires alongside it in **both** registration orders. RED control, the same tree with the tag removed: registering the unmigrated one first drops the minted one — an **order-sensitive** silent drop, invisible to a one-order probe. Test: `test-name-cannot-forge-a-minted-key` |
| `__id` is excluded from the compared subject, and it is the ONLY exclusion | `lib/search.nix`, binding `comparisonSubject`; a sealed continuation carrying `__id = throw …` still dedups — distinct pair ⇒ both fire, one value twice ⇒ one. `__id` is the accessor a consumer reads when it DEMANDS an identity, so where nothing is minted it IS the named refusal, and forcing it inside a bucket scan detonates the decision the refusal exists to permit. Test: `test-sealed-bucket-does-not-force-id` |
| Whether a key is a dedup key or a bucket label is read off `identityOf`, not off `__mint` a second time | `lib/search.nix`, binding `isExact` (`i: i ? minted`), consumed by `classify`. One `identityOf` read per continuation feeds both the key and its exactness, so the tagged sum keeps a single reader |
...
- **Palmer et al. (2024), *Intensional Functions*** — the search monad with continuation dedup (§3) in `lib/search.nix`, and the three eliminators `__functor` / `name` / `closure` (§2.2-2.3) in `lib/intensional.nix`. **Dedup is regime-dispatched, not name-keyed:** Fig. 5 is a conjunction over identity AND closure, and a program point is constant across a constructor's instances, so a name-only key merged behaviourally distinct continuations and dropped one silently. The key is now exact where an identity is minted and a **bucket** otherwise, with membership decided by Nix `==` on the reified value minus `__id`, and every key carries a regime tag so the three arms occupy disjoint spaces. `intensionalEq` in `lib/intensional.nix` is **still name-only** and is the remaining instance of the same defect — its repair is the constructor migration, which is not landed. Neither realizes Theorem 1: it is a preservation theorem about 𝜆ITS reduction and gen is not 𝜆ITS (`README.md` § *`intensionalEq`*).
